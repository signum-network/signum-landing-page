// src/components/RemoveCommitmentButton.tsx
...
export const RemoveCommitmentButton = (): JSX.Element => {
  ...
  const isInvalid = !publicKey || amount <= 0;
  ...
  const removeCommitment = async () => {
    ...
    try {
      await doTransaction((ledger) =>
        ledger.account.removeCommitment({
  ...
  return (
    <div className="card p-5">
      <div className="flex flex-col gap-3">
        <label
          htmlFor="remove-commitment-amount"
          className="text-sm font-medium text-neutral-700"
        >
          <strong>Remove Commitment (SIGNA)</strong>
        </label>

        <div className="flex items-center gap-2">
          <input
            id="remove-commitment-amount"
            type="text"
            inputMode="numeric"
            pattern="[0-9]*"
            value={value}
        ...
        <button
          ...
          className="inline-flex items-center justify-center rounded-full
                     ...
                     w-auto min-w-[120px] md:min-w-[140px] shrink-0 self-start
                     transition hover:bg-signum-darkblue
                     disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {isSending ? "Removing commitment…" : "Remove it"}
        </button>
